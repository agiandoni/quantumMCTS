...
class Annealer():
    def __init__(self, H0, H1, psi, sPath, reference_state = None):
        ...
        if not reference_state:
            self.reference_state = H1.groundstate()[1] #because .groundstate() returns a list [E0, psi0]
        else:
            self.reference_state = reference_state
      
        self.annealer_type = "QA" #For the game board to distinguish both types

    

    def anneal(self, coefficients = {}, times_list = None, extra_path_calculations = False):
        #coefficients is a dictionary {"c1":c1, ..., "cM":cM} of all of the coefficients that where seleted.
        ...
        if not extra_path_calculations:  
            ...
            #states_output = np.hstack(states, states_output)
    ...
    def anneal_manual(self, coefficients, extras = False):
        ...
        delta_t = self.sPath.T / (N_turns) #The length of the plateaus see notes
        ...
        for i in range(N_turns):
            ...
            #print(f"turn {i+1}/{N_turns}: evolving state with exp(-i*{delta_t}*H_t)")
            #print(H_t.eigenstates()[0])
            if extras:
                H_list.append(H_t)
...
class QAOA():
    def __init__(self, H0, H1, psi0, reference_state = None, qaoa_step_number = 1):
        ...
        self.n_bits = round(np.log(psi0.shape[0])/np.log(2)) #Make it so that it calculates this from the size of the operators/states it should also assert they all have the same size

        
   
    
    
    def run(self, coefficients, times_list = None, calculate_gradient = False, extra_path_calculations = False, simple = False, get_gradient = False):
        """
        ...
        calculate_gradient : Bool (False), optional
            If true the method also calculates the gradient of the QAOA ansatz analytically, it increases the computation time significantly
        ...
        """
        #We can write the H0 part of the QAOA ansatz as a rotation matrix instead of having to exponentiate every time!
        fast_x_rotation = True
        ...
        if get_gradient:
            ...
            gradient_2 = 2*np.concatenate((np.array(gamma_grad).imag, np.array(beta_grad).imag))
        ...
        if calculate_gradient:
            ...
            #We need to evolve <states[-1]|H1 backwards
            backward_states = [self.H1*states[-1]]
            
            for step in range(self.P):
                zphase = -1j*coefficients[0][self.P-1-step]*self.H1
                #xphase = -1j*coefficients[1][step]*self.H0
                backward_states.append(zphase.expm("sparse")*fast_x_rotation(coefficients[1][self.P-1-step])*backward_states[step]) #The order is changed!
                
                gamma_partial  = -1j * backward_states[step+1].overlap(self.H1*states[self.P-1-step])
                beta_partial   = -1j * backward_states[step].overlap(self.H0*states[self.P-step])
                
                gamma_partials.append(gamma_partial + gamma_partial.conjugate())
                beta_partials.append(beta_partial   + beta_partial.conjugate())
                
                #gamma_partials.append(2*np.real(gamma_partial))
                #beta_partials.append(2*np.real(beta_partial))
            
            gradient = [gamma_partials, beta_partials]
            return energies, fidelities, states, gradient, gradient_2
    ...
    def get_equivalent_QA_parameters(self, parameters):
        ...
        normalized_parameters = [i if i>0 else i+2*np.pi for i in parameters] #In case we used [-pi,pi) ranged parameters
        equiv_annealing_time = np.sum(normalized_parameters) #Do I need to half this because of the prefix in exp???
        self.annealing_time = equiv_annealing_time
        return equiv_annealing_time
        # step_path = np.array(parameters[0])
        # pass
...
def gradient_optimize(annealer, init_coeffs = None, jacobian = True):
    ...
    def get_reward(x):
        ...
        return energies[-1]
        #return annealer.run(coefficients=x) if annealer.annealer_type == "QAOA" else annealer.anneal(coefficients = x)
